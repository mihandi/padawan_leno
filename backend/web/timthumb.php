...
if (!defined('WATERMARK_OVERLAY_IMAGE')) {
    define('WATERMARK_OVERLAY_IMAGE', 'upload/watermark.png');
} // the watermark image
...
if (!defined('FILE_CACHE_TIME_BETWEEN_CLEANS')) {
    define('FILE_CACHE_TIME_BETWEEN_CLEANS', 365 * 86400);
}    // How often the cache is cleaned

if (!defined('FILE_CACHE_MAX_FILE_AGE')) {
    define('FILE_CACHE_MAX_FILE_AGE', 365 * 86400);
...
if (!defined('BROWSER_CACHE_MAX_AGE')) {
    define('BROWSER_CACHE_MAX_AGE', 365 * 86400);
...
class timthumb
{
    ...
    public function __construct()
    {
        ...
        if (BLOCK_EXTERNAL_LEECHERS && array_key_exists('HTTP_REFERER', $_SERVER) && (!preg_match(
        ...
        if ($this->isURL) {
            if (ALLOW_ALL_EXTERNAL_SITES) {
                ...
                foreach ($ALLOWED_SITES as $site) {
                    if ((strtolower(substr($this->url['host'], -strlen($site) - 1)) === strtolower(
                                ".$site"
        ...
        if ($this->isURL) {
            ...
            asort($arr);
            $this->cachefile = $this->cacheDirectory . '/' . FILE_CACHE_PREFIX . $cachePrefix . md5(
                    $this->salt . implode('', $arr) . $this->fileCacheVersion
                ) . FILE_CACHE_SUFFIX;
        } else {
            ...
            $this->cachefile = $this->cacheDirectory . '/' . FILE_CACHE_PREFIX . $cachePrefix . md5(
                    $this->salt . $this->localImageMTime . $_SERVER ['QUERY_STRING'] . $this->fileCacheVersion
                ) . FILE_CACHE_SUFFIX;
        }
    ...
    protected function cleanCache()
    {
        ...
        if (@filemtime($lastCleanFile) < (time(
                ) - FILE_CACHE_TIME_BETWEEN_CLEANS)) { //Cache was last cleaned more than 1 day ago
            $this->debug(
                1,
    ...
    protected function processImageAndWriteToCache($localImage)
    {
        ...
        if (strlen($canvas_color) == 3) { //if is 3-char notation, edit string into 6-char notation
            $canvas_color = str_repeat(substr($canvas_color, 0, 1), 2) . str_repeat(
                    substr($canvas_color, 1, 1),
                    2
        ...
        if ((IMAGETYPE_PNG == $origType || IMAGETYPE_GIF == $origType) && function_exists(
                'imageistruecolor'
            ) && !imageistruecolor($image) && imagecolortransparent($image) > 0) {
        ...
        if ($watermark == 1) {
            ...
            if (max($overlay_width / $new_width, $overlay_height / $new_height) > $max_watermark_ratio) {
                ...
                imagecopy(
                    $canvas,
                    $overlay_gd_image_p,
                    $new_width - $new_overlay_width - 20,
                    $new_height - $new_overlay_height - 20,
                    0,
                    0,
                    $new_overlay_width,
                    $new_overlay_height
                ...
                imagecopy(
                    $canvas,
                    $overlay_gd_image,
                    $new_width - $overlay_width - 20,
                    $new_height - $overlay_height - 20,
                    0,
                    0,
                    $overlay_width,
                    $overlay_height
        ...
        if (flock($fh, LOCK_EX)) {
            @unlink(
                $this->cachefile
            ); //rename generally overwrites, but doing this in case of platform specific quirks. File might not exist yet.
    ...
    protected function getLocalImagePath($src)
    {
        ...
        if (!$this->docRoot) {
            ...
            $file = preg_replace(
                ...
                '$1',
                $src
            ); //strip off any path info and just leave the filename.
            if (is_file($file)) {
    ...
    protected function serveWebshot()
    {
        ...
        $url = preg_replace(
            ...
            '',
            $url
        ); //RFC 3986 plus ()$ chars to prevent exploit below. Plus the following are also removed: @*!~#[]',
